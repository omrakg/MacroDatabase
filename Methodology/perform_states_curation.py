...
if RESET:
    if "Curated" in os.listdir(base_directory):
        if "States" in os.listdir(f"{base_directory}/Curated"):
            shutil.rmtree(f"{base_directory}/Curated/States")
if "Curated" not in os.listdir(base_directory):
    os.mkdir(f"{base_directory}/Curated")
if "States" not in os.listdir(f"{base_directory}/Curated"):
    os.mkdir(f"{base_directory}/Curated/States")

STATES = json.load(open(f"{structure_str}/states.json", "r"))
CURATION_LIST = json.load(open(f"{structure_str}/state_curation_list.json", "r"))
...
# Create state macro data
for state_1 in tqdm(dataset, desc="Collecting state options"):
    for key_1, value_1 in dataset[state_1].items():
        title_1 = value_1['title']

        if f"for the {state_1}" in title_1 or f"for {state_1}" in title_1 or f"in {state_1}" in title_1:
            if f"for the {state_1}" in title_1:
                item_1 = title_1.split(f" for the {state_1}")[0]
            elif f"for {state_1}" in title_1:
                item_1 = title_1.split(f" for {state_1}")[0]
            else:
                item_1 = title_1.split(f" in {state_1}")[0]
            ...
            similarities[item_1][name] = {state_1: value_1}

            for state_2 in dataset:
                if state_2 == state_1:
                    continue

                for key_2, value_2 in dataset[state_2].items():
                    title_2 = value_2['title']

                    if f"for the {state_2}" in title_2:
                        item_2 = title_2.split(f" for the {state_2}")[0]
                    elif f"for {state_2}" in title_2:
                        item_2 = title_2.split(f" for {state_2}")[0]
                    else:
                        item_2 = title_2.split(f" in {state_2}")[0]
